    ...
    for i=1:skipRate:length(t) #this makes animation scale ~1 sec/year with other conditions
        ...
        if sectionNum>1  #don't need to do this forever since they decay to 0 anyways
            for n=1:length(backData) #old list has +1 bodies #maybe change the condition here to just the limits from the last frame?? might fix problems
                p=plot!(backData[n][1]./1.5e11,backData[n][2]./1.5e11,label="",linewidth=3,linecolor=oldColorSymbols[n],linealpha=max.((1:Int(floor(skipRate/10)):oldI) .+ 10000 .- (oldI+i),1000)/10000)
            end
        end
        ...
        for n=1:nBodies
            p=plot!(xData[n][1:Int(floor(skipRate/10)):i]./1.5e11,yData[n][1:Int(floor(skipRate/10)):i]./1.5e11,label="",linewidth=3,linecolor=colorSymbols[n],linealpha=max.((1:Int(floor(skipRate/10)):i) .+ 10000 .- i,1000)/10000)
        end
