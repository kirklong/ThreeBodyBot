    ...
    while globalT<stopT && nBodies>1 #go until we reach given time or there's one giant black hole
        ...
        function firstStep(nBodies0,names0,colors0,stopT,globalT,stopCond2,dt) #try to find a first step that goes for a reasonable amount of time without immediate collisions
            ...
            while accept==false
                ...
                if t>(stopT/10) || iter>499 #don't do this forever, could probably be improved iteratively by modifying initial conditions slightly from longest yet step but I'm lazy
...
function plotSection(landscape,sectionNum,backData,oldI,oldColors,offsets,dt,nBodies,plotInfo,physInfo,labels,colors,maxFrame;slowDown=1)
    ...
    for i=1:skipRate:length(t) #this makes animation scale ~1 sec/year with other conditions
        ...
        if i>1
            ...
            if (limx[1]/oldLimx[1])<0.95 #frame shrunk more than 5%
            ...
            elseif (limx[1]/oldLimx[1])>1.05 #frame grew more than 5%
        ...
        dx,dy=getRatioRight(ratio,dx,dy) #just to be REALLY sure
        limx[2]=limx[1]+dx
        limy[2]=limy[1]+dy
        listInd+=1
        for n=1:nBodies
...
nTrials,plotList,tOffsets,physInfoList,namesList,colorsList,nBodiesList=getInterestingNBody(nBodies,names,colors,dt=0.0001,stopCond=[100,500],minTime=90,posRange=[-50,50])
...
plotAll(landscape,nBodiesList,plotList,physInfoList,namesList,colorsList,150,tOffsets,0.0001,slowDown=1)
