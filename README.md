...
"shit I should really improve this code now that people other than me might look at it" – [@ThreeBodyBot's inept parent](https://twitter.com/CaptainKirkLong)
...
You should be able to generate an animation right away without changing any settings simply by uncommenting the code plotting block at the bottom of `threeBodyProb.jl` and commenting out the big plotting code (see code comments for more details). If run this way an animation should be generated in the folder where the script was run from. For more advanced control over how the animations are generated you will need to generate the animation frames using the fancier (and much longer) method I just told you to comment out, and you will need to specify a file path where you want the frames to be saved at the end of both plotting loops in the `png` commands. That method also generates the frames ***significantly*** faster. It's on my "someday" to-do list to turn this into an actual binary "app" that you could run without even having Julia installed, but until that happens you have to get a little bit into the weeds, sorry! Once the frames are generated you need to stitch them together using something like FFmpeg – there are some example commands commented out in the julia script beneath the big plotting loops. If you want to make 3D versions of three-body problems do something similar with the `threeBody3D.jl` script. 
