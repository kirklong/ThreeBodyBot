...
There are several scripts in the `nbody` folder worth looking at if you are looking to create even more gravitational chaos. These will start occasionally popping up on the bot's Twitter (currently a 1 in 10 chance each day), and there are new instructions below that should get you started if you want to try to make one of these yourself! The camera implementation with these is still a little buggy though...so if you have great ideas on what should be done to improve it please do so and I will happily credit and merge your changes. 
...
You could also generate an animation right away without changing any settings simply by uncommenting the code plotting block at the bottom of `threeBodyProb.jl` and commenting out the big plotting code (see code comments for more details), and then running the script. For more advanced control over how the animations are generated you will need to generate the animation frames using the fancier (and much longer) method I just told you to comment out, and you will need to specify a file path where you want the frames to be saved at the end of both plotting loops in the `png` commands (by default it assumes you there is an empty subdirectory called "tmpPlots" it can place the png files). That method also generates the frames ***significantly*** faster. It's on my "someday" to-do list to turn this into an actual binary "app" that you could run without even having Julia installed, but until that happens you have to get a little bit into the weeds, sorry! Once the frames are generated you need to stitch them together using something like FFmpeg – there are some example commands commented out in the julia script beneath the big plotting loops, and there's a built-in function you can use (only tested on Linux though...) called ```makeAnim()```. If you want to make 3D versions of three-body problems do something similar with the `threeBody3D.jl` script. 
